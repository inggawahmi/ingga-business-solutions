...
import { FEATURED_CASE_STUDIES, CONFIDENTIALITY_NOTICE } from "@/data/caseStudies";
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-14">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-3">
          ...
          <h2 className="text-3xl sm:text-4xl font-extrabold text-[#101C24] tracking-tight">
            Pengalaman Menangani Sistem Bisnis Nyata
          </h2>
          <p className="text-sm sm:text-base text-[#667681] leading-relaxed font-normal">
            Pengalaman dari website publik hingga aplikasi internal dan ERP dengan workflow yang kompleks.
          </p>
        </div>
        ...
        <div className="space-y-8">
          {FEATURED_CASE_STUDIES.map((cs) => {
            ...
            return (
              <div
                key={cs.id}
                className="rounded-3xl p-6 sm:p-8 bg-[#F7F7F3] border border-[#DCE3E5] space-y-6 shadow-2xs"
              >
                {/* Card Top Banner */}
                <div className="flex flex-wrap items-center justify-between gap-3 pb-4 border-b border-[#DCE3E5]">
                  <div>
                    <span className="text-[11px] font-bold text-[#177568] uppercase tracking-wider block">
                      {cs.domain}
                    </span>
                    <h3 className="text-xl sm:text-2xl font-bold text-[#101C24] tracking-tight mt-0.5">
                      {cs.title}
                    </h3>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs sm:text-sm">
                  ...
                  {/* Tantangan */}
                  <div className="space-y-1.5 p-4 rounded-2xl bg-white border border-[#DCE3E5]">
                ...
                {/* Collapsible Technical Detail Accordion (Closed by default) */}
                <div className="pt-2 border-t border-[#DCE3E5]">
                  ...
                  >
                    <span>{isOpen ? "Tutup Detail Teknis" : "Lihat Detail Teknis"}</span>
                  ...
                  {isOpen && (
                    <div className="mt-3 p-4 rounded-2xl bg-white border border-[#DCE3E5] space-y-3 text-xs">
                      <div>
                        <strong className="text-[#101C24] block mb-1">Arsitektur:</strong>
                        <p className="text-[#667681]">{cs.technicalDetails.architecture}</p>
                      </div>
                      <div>
                        <strong className="text-[#101C24] block mb-1">Modul Utama:</strong>
        ...
        <div className="p-4 rounded-2xl bg-[#F7F7F3] border border-[#DCE3E5] text-center text-xs text-[#667681] max-w-2xl mx-auto flex items-center justify-center gap-2">
          ...
          <span>{CONFIDENTIALITY_NOTICE}</span>
        </div>
